...
public class CustomWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration((_, config) =>
        {
            ...
            var uploadsPath = Path.Combine(Path.GetTempPath(), $"padel-test-uploads-{testRunId}");
            var outputsPath = Path.Combine(Path.GetTempPath(), $"padel-test-outputs-{testRunId}");
            var logsPath = Path.Combine(Path.GetTempPath(), $"padel-test-logs-{testRunId}");

            config.AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["Storage:UploadsPath"] = uploadsPath,
                ["Storage:OutputsPath"] = outputsPath,
    ...
    public async Task ResetStateAsync()
    {
        ...
        DeleteFiles(configuration["Storage:UploadsPath"]);
        DeleteFiles(configuration["Storage:OutputsPath"]);
    }
